...
# YOLO ("yolov8n.pt")는 이미 사전 학습된 YOLOv8 nano모델입니다
model = YOLO("yolov8n.pt")

# model.predict() : 이미지에 대해 객체 탐지를 실행하는 함수
# source : 소스 (탐지할 이미지 소스)
# conf : 신뢰도 기준. (주어진 값 미만인 클래스는 버림)
# save : True일 때 탐지 결과가 그려진 이미지를 자동 저장해라.
# 반환값 : list[Result]  -> 여러개의 이미지를 분석 시킬 수 있기 때문에
#         결과 또한 여러개의 Result가 나올 수 있으므로

# results = model.predict(source="./images/Man_walking_dog_on_road.jpg",
#               conf=0.1, save=True)

# # results 는 리스트 타입이다!
# result = results[0]

# print("탐지 갯수 : ", len(result.boxes))
...
for i, r in enumerate(results, 1) :
  if len(r.boxes) > 0 :
    ...
    print(f"{i}번째 이미지의 탐지 클래스 번호 : {cls_id_0} => 이름 {r.names[cls_id_0]}, 신뢰도 {round(float(box.conf[0]), 1)}")
    print(f"픽셀 좌표 : {[int(v) for v in box.xyxy[0].tolist()]}")
    print(f"정규화 좌표 {[round(v, 3) for v in box.xywhn[0].tolist()]}")
    ...
    print(f"{i}번째 이미지에서 객체 탐지가 되지 않았습니다")
