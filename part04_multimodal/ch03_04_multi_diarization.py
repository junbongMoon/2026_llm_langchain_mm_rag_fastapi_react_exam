...
dialization_pipeline = Pipeline.from_pretrained(
  "pyannote/speaker-diarization-3.1",
    token=HF_TOKEN,          # use_auth_token 아님! pyannote 4.x는 token=
)

# wav 읽기 -> soundfile 라이브러리 사용(FFMPEG)
data, sample_rate = sf.read(AUDIO_PATH, dtype="float32")
# (time,) -> (1, time) "1은 MONO"
wavform = torch.from_numpy(data).unsqueeze(0) 

output = dialization_pipeline(
    {"waveform": wavform, "sample_rate": sample_rate},
